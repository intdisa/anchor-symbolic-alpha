...
This means the project has moved from an engineering bottleneck to a research bottleneck. A publishable NeurIPS direction needs a stronger method claim than "better local tuning of a single-asset timing system."
...
The paper should no longer rely only on single-asset timing.
