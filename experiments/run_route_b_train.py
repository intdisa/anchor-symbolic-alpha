...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run memory-bounded Route B training on a processed subset.")
    parser.add_argument("--episodes", type=int, default=4)
    parser.add_argument("--data-config", type=Path, default=Path("configs/route_b_smoke.yaml"))
    parser.add_argument("--training-config", type=Path, default=Path("configs/training_route_b_smoke.yaml"))
...
def parse_skill_list(raw: str) -> tuple[str, ...] | None:
    ...
    return resolve_route_b_skill_aliases(
        skills
        or (
            "quality_solvency",
        )
...
def main() -> None:
    ...
    report_path = output_dirs["reports"] / "route_b_train_summary.json"
    factors_path = output_dirs["factors"] / "route_b_champions.json"
    payload = {
