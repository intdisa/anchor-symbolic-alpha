...
def normalize_xsc_us_equities(
    source_root: str | Path = DEFAULT_XSC_SOURCE_ROOT,
    output_root: str | Path = DEFAULT_US_EQUITIES_WRDS_ROOT,
...
def _normalize_crsp(
    ...
    with gzip.open(daily_output_path, "wt", encoding="utf-8", newline="") as handle:
        for chunk_index, chunk in enumerate(
            ...
            if chunk_index % 20 == 0:
                _log(f"[us-equities] crsp chunks={chunk_index} rows_written={total_daily_rows}")
    ...
    _log(f"[us-equities] finish crsp rows_written={total_daily_rows} names_rows={len(names)}")
...
def _normalize_ccm(source_path: Path, output_path: Path) -> int:
    _log(f"[us-equities] start ccm source={source_path}")
    ...
    _log(f"[us-equities] finish ccm rows_written={len(frame)}")
    return len(frame)
...
def _normalize_compustat_csv(
    ...
    _log(f"[us-equities] start compustat source={source_path} chunksize={chunksize}")
    with gzip.open(output_path, "wt", encoding="utf-8", newline="") as handle:
        for chunk_index, chunk in enumerate(
            ...
            if chunk_index % 10 == 0:
                _log(f"[us-equities] compustat chunks={chunk_index} rows_written={total_rows} source={source_path.name}")
    if not header_written:
        raise ValueError(f"No rows written for {source_path}.")
    _log(f"[us-equities] finish compustat rows_written={total_rows} source={source_path.name}")
    return total_rows
